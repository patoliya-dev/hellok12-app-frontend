  ...
  const renderCalendarGrid = () => {
    ...
    for (let day = 1; day <= totalDays; day++) {
      ...
      cells.push(
        ...
        >
          <div
            className={`flex items-center justify-center h-7 w-7 text-sm ${isToday ? "bg-brand-blue text-white rounded-full" : ""}`}
          >
            {day}
          </div>
          <div className="mt-1 space-y-1">
            {lessonsForDay.map((lesson) => (
              <div key={lesson.id} className="flex items-center">
                <span
                  className={`w-1.5 h-1.5 rounded-full mr-2 ${colorMap[lesson.color]?.dot}`}
                ></span>
    ...
    <div className="bg-card rounded-lg border border-border">
      ...
      <div className="grid grid-cols-7">
        {daysOfWeek.map((day) => (
          <div
            key={day}
            className="p-3 text-center text-sm font-medium text-muted-foreground border-b border-r border-border"
          >
            {day}
          </div>
        ))}
